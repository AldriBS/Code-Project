...
// ==================== NEW: PERIODIC RFID RESET WHEN SENSOR DISABLED ====================
unsigned long lastRFIDPeriodicReset = 0;
const unsigned long RFID_PERIODIC_RESET_INTERVAL = 5000;  // Reset setiap 5 detik
// =======================================================================================
...
void readDoorSensor() {
  // ==================== PERIODIC RFID RESET SAAT SENSOR DISABLED ====================
  if (!sensorEnabled) {
    // Lakukan periodic reset RFID setiap 5 detik untuk mencegah hang
    ...
    return;  // Skip pembacaan sensor jika disabled
  }
  // =================================================================================
...
// ==================== UPDATE: LCD Door Status dengan Sensor Status ====================
void updateLCDDoorStatus() {
  lcd.setCursor(0, 2);

  if (!sensorEnabled) {
    // Tampilkan status sensor disabled
    lcd.print("Door Sensor: DISABLED");
  } else {
    // Tampilkan status door normal
    lcd.print("Door: ");
    if (doorOpen) {
      lcd.print("OPEN          ");
    } else {
...
void lcdShowIdle() {
  ...
  updateLCDDoorStatus();  // Akan otomatis menampilkan status yang sesuai
  resetRFID();
}
...
void handleSensorToggle() {
  ...
  // ==================== UPDATE LCD SETELAH PERUBAHAN STATUS ====================
  if (!scanMode) {
    updateLCDDoorStatus();  // Update tampilan LCD dengan status baru
  }
  // =============================================================================

  if (newStatus) {
    // Sensor di-enable: baca status door dan reset RFID jika perlu
    ...
    // Sensor disabled: reset RFID untuk memastikan siap digunakan
    if (!rfidReady) {
      resetRFID();
    }
    lastRFIDPeriodicReset = millis();  // Reset timer periodic
  }
