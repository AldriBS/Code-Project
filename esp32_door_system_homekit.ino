...
const char* homekit_name = "Door Lock Aldri";
const char* homekit_setup_code = "466-37-726";
// Objects
...
const unsigned long RFID_PERIODIC_RESET_INTERVAL = 5000;
...
struct DoorLockAccessory : Service::LockMechanism {
  ...
  void loop() {
    if (autoLockPending && millis() - unlockStartTime >= AUTO_LOCK_DURATION) {
      lockDoor();
      lockCurrentState->setVal(1);
...
void unlockDoor(String source) {
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("Unlock from:");
  lcd.setCursor(0, 1); lcd.print(source);
  rfidReady = false;
  setRelay(true);
  writeLog(source, source == "HomeKit" ? "HOMEKIT-UNLOCK" : "RFID-UNLOCK", true);
  beepOK();
...
void writeLog(const String& name, const String& uid, bool success) {
  ...
  if (f) {
    ...
    String entry = name + "," + uid + "," + String(dateStr) + "," + String(timeStr) + "," + timestamp + "," + status;
    f.println(entry);
    f.close();
    logSerial("[LOG] " + entry);
  }
  ...
  if (f) {
    ...
    while (f.available()) {
      ...
      if (idx1 > 0 && idx2 > 0 && idx3 > 0 && idx4 > 0 && idx5 > 0) {
        ...
        if (!first) json += ",";
        json += "{\"name\":\"" + name + "\",\"uid\":\"" + uid + "\",\"timestamp\":" + timestamp + ",\"status\":\"" + status + "\"}";
        first = false;
      }
    }
    f.close();
...
void handleRTCSync() {
  ...
  logSerial("[OK] RTC synced with timezone offset: " + String(timezoneOffsetMinutes) + " minutes (" + String(timezoneOffsetMinutes / 60) + " hours)");
  logSerial("[OK] UTC timestamp: " + String(timestamp));
  logSerial("[OK] Local timestamp: " + String(localTimestamp));
  logSerial("[OK] RTC now: " + String(now.year()) + "-" + String(now.month()) + "-" + String(now.day()) + " " + String(now.hour()) + ":" + String(now.minute()) + ":" + String(now.second()));
...
void handleDoorControl() {
  ...
  if (action == "unlock") {
    ...
    unlockDoor(userName);
    if (doorLockAccessory) {
      doorLockAccessory->lockCurrentState->setVal(0);
      doorLockAccessory->lockTargetState->setVal(0);
    }
  } else if (action == "lock") {
    server.send(200, "application/json", "{\"success\":true,\"status\":\"locked\"}");
    lockDoor();
    if (doorLockAccessory) {
      doorLockAccessory->lockCurrentState->setVal(1);
      doorLockAccessory->lockTargetState->setVal(1);
    }
  } else {
...
void handleSensorToggle() {
  ...
  if (newStatus) {
    ...
    logSerial("[Sensor] Door state: " + String(doorOpen ? "OPEN" : "CLOSED"));
    if (!doorOpen && !rfidReady) {
      resetRFID();
    }
  } else {
    if (!rfidReady) {
...
void handleHomeKitStatus() {
  logSerial("[Web] GET /homekit/status");
  String json = "{";
  json += "\"paired\":" + String(homeSpan.isPaired() ? "true" : "false") + ",";
  json += "\"setupCode\":\"" + String(homekit_setup_code) + "\",";
  json += "\"name\":\"" + String(homekit_name) + "\",";
  json += "\"connections\":" + String(homeSpan.getConnectionCount());
  json += "}";
  server.send(200, "application/json", json);
}
void handleHomeKitReset() {
  logSerial("[Web] POST /homekit/reset");
  homeSpan.processSerialCommand("A");
  server.send(200, "application/json", "{\"success\":true,\"message\":\"HomeKit pairings cleared\"}");
  logSerial("[HomeKit] All pairings cleared - ready for re-pairing");
}
void handleRestart() {
...
void setupWebServer() {
  ...
  server.on("/homekit/status", handleHomeKitStatus);
  server.on("/homekit/reset", HTTP_POST, handleHomeKitReset);
  server.on("/api/files/list", handleFilesList);
  server.on("/api/files/download", handleFilesDownload);
  server.on("/api/files/upload", HTTP_POST, []() { server.send(200); }, handleFilesUpload);
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n\n=== ESP32 Door System v4.0 + HomeKit ===");
  serialBuffer = "=== ESP32 Door System with HomeKit Started ===\n";
  esp_task_wdt_deinit();
  ...
  esp_task_wdt_reset();
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("Init HomeKit...");
  homeSpan.begin(Category::Locks, homekit_name);
  homeSpan.setControlPin(0);
  homeSpan.setStatusPin(2);
  homeSpan.setSketchVersion("4.0");
  homeSpan.setWifiCredentials(WiFi.SSID().c_str(), WiFi.psk().c_str());
  homeSpan.setQRID(homekit_setup_code);
  new SpanAccessory();
  new Service::AccessoryInformation();
  new Characteristic::Identify();
  new Characteristic::Manufacturer("Aldri");
  new Characteristic::SerialNumber("ESP32-001");
  new Characteristic::Model("Door Lock v4.0");
  new Characteristic::FirmwareRevision("4.0.0");
  doorLockAccessory = new DoorLockAccessory();
  logSerial("[OK] HomeKit initialized");
  logSerial("[HomeKit] Setup Code: " + String(homekit_setup_code));
  lcd.setCursor(0, 1); lcd.print("HomeKit Ready");
  lcd.setCursor(0, 2); lcd.print("Code: " + String(homekit_setup_code));
  delay(3000);
  esp_task_wdt_reset();
  if (WiFi.status() == WL_CONNECTED) {
